...
for rid in used_routes:
    r=route_raw.get(rid,{});tids=route_trips[rid];heads=sorted({trips[t]['headsign'] for t in tids if trips[t]['headsign']});sample=max(tids,key=lambda t:len(trip_sequences[t]))
...
for sid,s in stops.items():
    s['routes']=sorted(stop_routes[sid]);s['times']={rid:sorted(set(route_times[(sid,rid)])) for rid in s['routes'] if route_times[(sid,rid)]}
hubs={}
